<template>
  <div class="matches-table">
    ...
    <template v-else>
      ...
      <DatePicker v-model="dateFrom" @date-change="onDateChange" />
...
export default {
  ...
  methods: {
    async fetchData() {
      ...
      this.error = null;
      try {
        // пытается получить данные из API
        const endpoint = (this.entityType = "api/v4/teams");
        const response = await api.get(endpoint); // await api.get(...) используется для получения данных с сервера

        const dataKey = (this.entityType = "teams");
    ...
    getScoreText(item) {
      return item.score.fullTime.homeTeam !== null &&
        item.score.fullTime.awayTeam !== null
        ? `${item.score.fullTime.homeTeam} - ${item.score.fullTime.awayTeam}`
        : "-";
    },
    getHomeScore(match) {
      return match.score.fullTime.homeTeam !== null
        ? match.score.fullTime.homeTeam
        : "-";
    },
    getAwayScore(match) {
      return match.score.fullTime.awayTeam !== null
